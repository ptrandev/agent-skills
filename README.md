...
Getting a browser runs from [`ui-walkthrough/driver.md`](ui-walkthrough/driver.md). Port lanes, the per-lane lock, and `browse` daemon scoping run from [`ui-walkthrough/concurrency.md`](ui-walkthrough/concurrency.md). Stack boot and the persona hold spec run from [`ui-walkthrough/stack.md`](ui-walkthrough/stack.md). The component ledger, the mount ladder, `--surfaces` semantics, and fixture derivation run from [`ui-walkthrough/coverage.md`](ui-walkthrough/coverage.md). The three capture passes run from [`ui-walkthrough/capture.md`](ui-walkthrough/capture.md), which is also the only file the capture sub-agent reads. Attribution, the numeric blocker thresholds, and the judged pass run from [`ui-walkthrough/evaluate.md`](ui-walkthrough/evaluate.md). Publishing and linking the screenshots runs from [`ui-walkthrough/evidence-hosting.md`](ui-walkthrough/evidence-hosting.md). Posting, the report, and teardown run from [`ui-walkthrough/post-and-report.md`](ui-walkthrough/post-and-report.md). The recording contract is [`ui-walkthrough/opencap.md`](ui-walkthrough/opencap.md). The `--embedded` return shape is [`ui-walkthrough/embedded.md`](ui-walkthrough/embedded.md). GitHub calls go through the transport shared with `/review-pr`, [`review-pr/github-transport.md`](review-pr/github-transport.md).
