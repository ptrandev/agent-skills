...
| [`babysit-prs`](skills/babysit-prs/SKILL.md) | Handles safe review feedback on open PRs you authored and leaves ambiguous threads open. Verify commands and the auto-resolve bar in [`repos.md`](skills/babysit-prs/repos.md). | `/babysit-prs [PR# ...] [--repo owner/name]` |
...
| [`mockup`](skills/mockup/SKILL.md) | Builds a self-contained interactive HTML mockup before implementation. Cached tokens per product in [`atllas-agents-portal.md`](skills/mockup/references/atllas-agents-portal.md) and [`neema-simple-hyzl.md`](skills/mockup/references/neema-simple-hyzl.md), the self-check in [`sweep.js`](skills/mockup/references/sweep.js). | `/mockup [ticket\|plan.md\|description]`<br>`--variants=N`, `--out=path`, `--publish` |
| [`ui-walkthrough`](skills/ui-walkthrough/SKILL.md) | Tests a PR's UI at several widths and posts screenshot evidence to GitHub. Windows uses [WSL2](skills/ui-walkthrough/windows.md). Target rules in [`target-selection.md`](skills/ui-walkthrough/target-selection.md), the Hyzl fixture stack in [`hyzl-stack.md`](skills/ui-walkthrough/hyzl-stack.md). | `/ui-walkthrough [PR#\|URL]`<br>`--author\|--reviewer`, `--target=e2e\|dev`, `--no-post` |
