...
| [`mockup`](skills/mockup/SKILL.md) | Builds a self-contained interactive HTML mockup before implementation. Cached Atllas tokens in [`atllas-agents-portal.md`](skills/mockup/references/atllas-agents-portal.md), the self-check in [`sweep.js`](skills/mockup/references/sweep.js). | `/mockup [ticket\|plan.md\|description]`<br>`--variants=N`, `--out=path`, `--publish` |
